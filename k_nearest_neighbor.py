...
class KNearestNeighbor():    
    def __init__(self, n_neighbors, distance_measure='euclidean', aggregator='mode'):
        ...
        self.k_nns = None #list of k_nearest neighbors labels
        raise NotImplementedError()


    def fit(self, train, features, targets):
        ...
        """
        distList = []
        for label, val in train:
            if self.distance_measure == 'euclidean':
                distList.append((label, euclidean(val, features)))
            elif self.distance_measure == 'cosim':
                distList.append((label, cosim(val, features)))
            #sorting by lowest distance x[1]
            distList.sort(key = lambda x: x[1])
            #getting just the labels i[0], distances are i[1]
            self.k_nns = [i[0] for i in distList[:self.n_neighbors]]
            return self.k_nns
    ...
    def predict(self, features, ignore_first=False):
        ...
        if self.aggregator == 'mode':
            probLabel = max(set(self.k_nns), key = self.k_nns.count) # 3
        elif self.aggregator == 'median':
            probLabel = np.median(self.k_nns, axis = 0)
        elif self.aggregator == 'mean':
            probLabel = np.mean(self.k_nns, axis = 1)
